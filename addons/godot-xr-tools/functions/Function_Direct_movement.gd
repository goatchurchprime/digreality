...
func _ready():
	...
	print("Funcgioncontrollermcalled", origin_node, camera_node, enabled, controller, controller.get_is_active())
	...
	if controller.get_is_active():
		...
		if isflying:	
			if controller.is_button_pressed(fly_move_button_id):
				...
				var fly_speed_fac = 0.3 if curr_transform.origin.y < 1.9 else 3
				velocity = controller.global_transform.basis.z.normalized() * -delta * max_speed * fly_speed_fac * ARVRServer.world_scale
		...
		# move_type == MOVEMENT_TYPE.move_and_strafe
			...
			velocity = $KinematicBody.move_and_slide(velocity, Vector3(0.0, 1.0, 0.0), true)
...
func _input(event):
	...
	if Input.is_key_pressed(KEY_R):
		...
		if controller.get_is_active():
			...
			var forwards_backwards = controller.get_joystick_axis(1)
			# if fly_action_button_id is released it deactivates the FLY MODE
			print("con", controller.is_button_pressed(fly_activate_button_id), left_right, forwards_backwards)
		else:
			print("RR not active")
